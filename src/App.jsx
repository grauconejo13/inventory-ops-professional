...
export default function App() {
  const [assets, setAssets] = useState(() => readStored('inventory-ops-assets', data))
  const [events, setEvents] = useState(() => readStored('inventory-ops-audit', []))
  const [query, setQuery] = useState('')
  const [kind, setKind] = useState('All')
  const [chosen, setChosen] = useState(data[1].id)
  const [form, setForm] = useState(null)
  const [archived, setArchived] = useState(false)
  useEffect(() => window.localStorage.setItem('inventory-ops-assets', JSON.stringify(assets)), [assets])
  useEffect(() => window.localStorage.setItem('inventory-ops-audit', JSON.stringify(events)), [events])
  ...
  const save = (event) => {
    ...
    if (form.id) {
      setAssets(assets.map((item) => item.id === form.id ? { ...form, quantity: Number(form.quantity), cost: Number(form.cost) } : item))
      setEvents([{ id: crypto.randomUUID(), action: 'Updated', asset: form.name, time: new Date().toLocaleString() }, ...events])
    } else {
      const item = { ...form, id: 'A' + (1245590 + assets.length), quantity: Number(form.quantity), cost: Number(form.cost) }
      setAssets([...assets, item]); setChosen(item.id)
      setEvents([{ id: crypto.randomUUID(), action: 'Created', asset: item.name, time: new Date().toLocaleString() }, ...events])
    }
    setForm(null)
  }
  const archive = () => {
    setAssets(assets.map((item) => item.id === selected.id ? { ...item, status: 'Archived' } : item))
    setEvents([{ id: crypto.randomUUID(), action: 'Archived', asset: selected.name, time: new Date().toLocaleString() }, ...events])
    setChosen('')
  }
  return <div className="app">
    ...
    <main id="inventory"><header>Home <span>/</span> Inventory <button className="create" onClick={() => setForm(blank)}>＋ Add asset</button></header>
      ...
      <section className="audit" aria-labelledby="audit-title"><div><small>LOCAL AUDIT TRAIL</small><h2 id="audit-title">Recent activity</h2></div>{events.length ? <ol>{events.slice(0, 5).map((event) => <li key={event.id}><b>{event.action}</b> {event.asset}<time>{event.time}</time></li>)}</ol> : <p>Changes made in this browser will be recorded here.</p>}</section>
    </main>
